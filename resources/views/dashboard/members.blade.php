        ...
        <tbody>
            @forelse($members as $member)
                <tr>
                    ...
                    <td>
                        <form action="{{ route('dashboard.users.block', $member->id) }}" method="POST" onsubmit="return confirm('{{ $member->is_blocked ? 'Buka blokir' : 'Blokir' }} member ini?')">
                            @csrf @method('PUT')
            ...
            @empty
                <tr><td colspan="6" class="empty-state">Belum ada member.</td></tr>
            @endforelse
        </tbody>
    </table>
